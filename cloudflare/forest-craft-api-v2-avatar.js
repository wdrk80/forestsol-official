...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      console.error(err);
      return json({ ok: false, error: err.message || "Internal error" }, Number(err && err.status) || 500, cors);
    }
  }
};
...
function corsHeaders(req) {
  ...
  const ok = o === "https://forestsol.jp" || o === "https://www.forestsol.jp" || /^https?:\/\/(localhost|127\.0\.0\.1)(:\d+)?$/.test(o);
  return {
    "Access-Control-Allow-Origin": ok ? o : "https://forestsol.jp",
    "Vary": "Origin",
...
function addDays(iso, d) { const x = new Date(iso); x.setUTCDate(x.getUTCDate() + d); return x.toISOString(); }
function b64url(bytes) { let s = ""; for (const b of bytes) s += String.fromCharCode(b); return btoa(s).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, ""); }
function fromB64url(s) { s = s.replace(/-/g, "+").replace(/_/g, "/"); while (s.length % 4) s += "="; const r = atob(s), o = new Uint8Array(r.length); for (let i = 0; i < r.length; i++) o[i] = r.charCodeAt(i); return o; }
...
async function readJson(req) { try { return await req.json(); } catch { const e = new Error("JSONが不正です"); e.status = 400; throw e; } }
function pub(u) { return { id: u.id, username: u.username, display_name: u.display_name, avatar_url: u.avatar_url || "", bio: u.bio || "", role: u.role || "user", created_at: u.created_at || null }; }
function authError() { return Object.assign(new Error("ログインが必要です"), { status: 401 }); }
function forbidden() { return Object.assign(new Error("この操作を行う権限がありません"), { status: 403 }); }
function ownerOrAdmin(u, p) { if (!u || (!['admin', 'moderator'].includes(u.role) && u.id !== p.user_id)) throw forbidden(); }
function bucketFor(env, t) { if (t === "skin") return env.SKINS; if (t === "model") return env.MODELS; return env.ASSETS; }
function validStorage(t) { return ["skin", "asset", "model"].includes(t); }
...
async function sessionUser(req, env, required = false) {
  const m = (req.headers.get("Authorization") || "").match(/^Bearer\s+(.+)$/i);
  if (!m) { if (required) throw authError(); return null; }
  const th = await sha256Text(m[1]);
  const row = await env.DB.prepare(`SELECT u.*,s.id AS session_id FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token_hash=? AND s.expires_at>? AND u.status='active'`).bind(th, now()).first();
...
async function route(req, env, url) {
  ...
  if (path === "/" && req.method === "GET") return json({ ok: true, name: "Forest Craft API", version: "2.2-avatar-upload", auth: "registered-users-only", login: "email" });
  ...
  if (path === "/auth/me" && req.method === "GET") { const u = await sessionUser(req, env, true); return json({ ok: true, user: pub(u) }); }
  ...
  let m = path.match(/^\/avatars\/([^/]+)$/);
  if (m && req.method === "GET") return serveAvatar(env, decodeURIComponent(m[1]));

  m = path.match(/^\/users\/([^/]+)$/); if (m && req.method === "GET") return getUser(env, decodeURIComponent(m[1]));
...
async function register(req, env) {
  const b = await readJson(req), email = String(b.email || "").trim().toLowerCase(), username = String(b.username || "").trim(), display = String(b.display_name || "").trim(), password = String(b.password || "");
  if (!validEmail(email)) return json({ ok: false, error: "正しいメールアドレスを入力してください" }, 400);
  ...
  const e1 = await env.DB.prepare("SELECT id FROM users WHERE lower(email)=lower(?)").bind(email).first(); if (e1) return json({ ok: false, error: "そのメールアドレスは登録済みです" }, 409);
  const e2 = await env.DB.prepare("SELECT id FROM users WHERE lower(username)=lower(?)").bind(username).first(); if (e2) return json({ ok: false, error: "そのユーザー名は使用されています" }, 409);
  const salt = randomToken(16), hash = await hashPassword(password, salt), id = "user_" + crypto.randomUUID(), t = now();
  await env.DB.prepare(`INSERT INTO users(id,username,display_name,avatar_url,role,status,password_hash,password_salt,bio,email,email_verified,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(id, username, display, "", "user", "active", hash, salt, "", email, 0, t, t).run();
  const token = await createSession(env, id, req), u = await env.DB.prepare("SELECT * FROM users WHERE id=?").bind(id).first();
  return json({ ok: true, token, user: pub(u) }, 201);
}

async function login(req, env) {
  const b = await readJson(req), email = String(b.login || b.email || "").trim().toLowerCase(), password = String(b.password || "");
  const u = await env.DB.prepare(`SELECT * FROM users WHERE lower(email)=lower(?) AND status='active'`).bind(email).first();
  if (!u || !u.password_hash || !u.password_salt) return json({ ok: false, error: "メールアドレスまたはパスワードが違います" }, 401);
  const hash = await hashPassword(password, u.password_salt); if (hash !== u.password_hash) return json({ ok: false, error: "メールアドレスまたはパスワードが違います" }, 401);
  const token = await createSession(env, u.id, req); return json({ ok: true, token, user: pub(u) });
}

async function createSession(env, userId, req) {
  const token = randomToken(32), hash = await sha256Text(token), t = now(), id = "sess_" + crypto.randomUUID();
  await env.DB.prepare("INSERT INTO sessions(id,user_id,token_hash,created_at,expires_at,last_seen_at,user_agent,ip_hint) VALUES(?,?,?,?,?,?,?,?)").bind(id, userId, hash, t, addDays(t, SESSION_DAYS), t, (req.headers.get("User-Agent") || "").slice(0, 180), (req.headers.get("CF-Connecting-IP") || "").split(".").slice(0, 2).join(".")).run();
  return token;
}

async function logout(req, env) {
  const m = (req.headers.get("Authorization") || "").match(/^Bearer\s+(.+)$/i);
  if (m) await env.DB.prepare("DELETE FROM sessions WHERE token_hash=?").bind(await sha256Text(m[1])).run();
  return json({ ok: true });
}

async function updateMe(req, env) {
  const u = await sessionUser(req, env, true), b = await readJson(req), name = String(b.display_name ?? u.display_name).trim(), bio = String(b.bio ?? u.bio ?? "").trim(), avatar = String(b.avatar_url ?? u.avatar_url ?? "").trim();
  if (!name || name.length > 40) return json({ ok: false, error: "表示名は1〜40文字です" }, 400);
  if (bio.length > 500) return json({ ok: false, error: "自己紹介は500文字までです" }, 400);
  await env.DB.prepare("UPDATE users SET display_name=?,bio=?,avatar_url=?,updated_at=? WHERE id=?").bind(name, bio, avatar, now(), u.id).run();
...
async function uploadAvatar(req, env, url) {
  ...
  if (!AVATAR_TYPES.has(mime)) return json({ ok: false, error: "JPG・PNG・WebP画像を選んでください" }, 415);
  const bytes = await req.arrayBuffer();
  if (!bytes.byteLength) return json({ ok: false, error: "画像が空です" }, 400);
  if (bytes.byteLength > AVATAR_MAX_BYTES) return json({ ok: false, error: "アイコン画像は3MBまでです" }, 413);
  const key = `avatars/${u.id}/avatar`;
  await env.ASSETS.put(key, bytes, { httpMetadata: { contentType: mime } });
  const avatarUrl = `${url.origin}/avatars/${encodeURIComponent(u.id)}?v=${Date.now()}`;
  await env.DB.prepare("UPDATE users SET avatar_url=?,updated_at=? WHERE id=?").bind(avatarUrl, now(), u.id).run();
  const fresh = await env.DB.prepare("SELECT * FROM users WHERE id=?").bind(u.id).first();
  return json({ ok: true, user: pub(fresh) }, 201);
}
...
async function deleteAvatar(req, env) {
  ...
  const fresh = await env.DB.prepare("SELECT * FROM users WHERE id=?").bind(u.id).first();
  return json({ ok: true, user: pub(fresh) });
}

async function serveAvatar(env, userId) {
  const user = await env.DB.prepare(`SELECT id,status FROM users WHERE id=? AND status='active'`).bind(userId).first();
  if (!user) return json({ ok: false, error: "ユーザーが見つかりません" }, 404);
...
async function createPost(req, env) {
  ...
  if (!title || title.length > 120) return json({ ok: false, error: "タイトルは1〜120文字です" }, 400);
  if (!validCategory(category)) return json({ ok: false, error: "カテゴリが不正です" }, 400);
  const visibility = validVisibility(b.visibility) ? b.visibility : "public", status = validStatus(b.status) ? b.status : "published", id = "post_" + crypto.randomUUID(), t = now(), desc = String(b.description || "").slice(0, 5000), tags = JSON.stringify(Array.isArray(b.tags) ? b.tags.slice(0, 20).map(x => String(x).slice(0, 40)) : []), classic = category === "skin" ? (b.classic_slim === "slim" ? "slim" : "classic") : null;
  await env.DB.prepare("INSERT INTO posts(id,user_id,title,description,category,tags_json,visibility,status,classic_slim,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)").bind(id, u.id, title, desc, category, tags, visibility, status, classic, t, t).run();
  await env.DB.prepare("INSERT OR IGNORE INTO post_stats(post_id,download_count,view_count,favorite_count) VALUES(?,0,0,0)").bind(id).run();
  return json({ ok: true, post: await env.DB.prepare("SELECT * FROM posts WHERE id=?").bind(id).first() }, 201);
}
async function getPost(req, env, id) { const p = await env.DB.prepare(POST_SELECT + " WHERE p.id=?").bind(id).first(); if (!p) return json({ ok: false, error: "作品が見つかりません" }, 404); if (p.visibility !== "public" || p.status !== "published") ownerOrAdmin(await sessionUser(req, env, false), p); else env.DB.prepare("UPDATE post_stats SET view_count=view_count+1 WHERE post_id=?").bind(id).run().catch(() => {}); p.files = (await env.DB.prepare("SELECT id,post_id,storage_type,file_role,object_key,filename AS original_filename,mime AS mime_type,size FROM post_files WHERE post_id=? ORDER BY file_role,id").bind(id).all()).results || []; return json({ ok: true, post: p }); }
